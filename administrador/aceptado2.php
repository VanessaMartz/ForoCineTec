<?php
   ...
   if(!empty($_POST)){
        $idforo = $_POST['idf'];
        $query = mysqli_query($conn,"UPDATE foros SET permiso = 'aceptado' WHERE idforo = '$idforo'");
        
    if($query){
        header("Location: accesos.php");
    }else{
        $alert = '<div class="alert alert-danger text-center" role="alert">No se pudo eliminar</div>';
    }
}
...
<h2>Confirmar</h2>
      ...
      <form method ="post" >
       <div class="form-group text-center">
          <label for="idf" class="h2">¿Desea confirmar esta acción?</label>
